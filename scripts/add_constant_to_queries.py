...
def add_fixed_cst_to_query(query: str, df: pd.DataFrame, seed) -> str:
    ...
    for m in re.finditer(r"(\?c[0-9]+)", str(query)):
        ...
        if len(df[x_c]) > 0:
            ...
            column = re.sub(r'\b\[([A-Za-z]+_[0-9]+)"\]\b', r'"\1"', column)
            column = re.sub(r"\b(http://example.org/(s[0-9]+|federated_shop)/[A-Za-z]+(_s[0-9]+)?_[0-9]+)\b", r'<\1>',
                            column)
...
def get_ss_query(query: str) -> str:
    ...
    for i in range(0, len(triples)):
        if "sameAs" in triples[i][1]:
            # quick and dirty : we know that sameAs are always inside OPTIONAL in our queries...
            source_selection_query += "OPTIONAL{GRAPH ?tp" + str(i) + " { " + " ".join(triples[i]) + " }} . \n"
        else:
